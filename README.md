...
**Development (run API with env vars loaded):**
...
```bash
cd app && source ../.venv/bin/activate && export $(cat ../.env.local | grep -v '^#' | xargs) && uvicorn main:app --reload
```

**Important:** `.env.local` must be created first with JWT keys (see `docs/KEY_MANAGEMENT.md`).

Available at `http://localhost:8000`. Interactive docs at `http://localhost:8000/docs`.

**Run tests** (from repo root, env vars auto-loaded by conftest.py):
...
For development, create `.env.local` with base64-encoded JWT keys (never commit):

```bash
# .env.local (gitignored)
JWT_PRIVATE_KEY_B64=MIIEvgIBADANBgkqhkiG9w0BAQEF...
JWT_PUBLIC_KEY_B64=MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8A...
```

For production, set env vars from secret manager (AWS Secrets Manager, GCP Secret Manager, Vault). See `docs/KEY_MANAGEMENT.md`.
